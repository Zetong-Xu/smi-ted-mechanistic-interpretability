...
def extract_hidden_states_per_layer(model, tokenizer, smiles_list, max_molecules=2000):
    ...
    for smi in tqdm(smiles_list, desc="Extracting hidden states"):
        ...
        # Store labels with molecule index to enable molecule-level train/test split
        for prop in props:
            prop['molecule_idx'] = processed  # track which molecule each atom belongs to
...
def run_linear_probing(layer_embeddings, atom_labels):
    ...
    for task_name, labels in probing_tasks.items():
        ...
        # Molecule-level train/test split to prevent data leakage
        # Original code split at atom level, allowing atoms from the same molecule
